...
@section('content')
<article><section class="corporate-hero"><div class="corporate-container"><p class="eyebrow">{{ $payload['category'] }}</p><h1>{{ $payload['title'] }}</h1><p class="lead">{{ $payload['short_answer']??'' }}</p></div></section><div class="corporate-container corporate-detail"><p><a href="{{ route('knowledge.'.$entry->type.'.index') }}">← {{ \App\Services\KnowledgeContent::TYPES[$entry->type] }}</a></p>
<p>By {{ $payload['author']??'—' }} · Reviewed by {{ $payload['reviewer']??'—' }} @if($publishedDate) · Published {{ $publishedDate }} @endif @if($updatedDate) · Updated {{ $updatedDate }} @endif</p>
