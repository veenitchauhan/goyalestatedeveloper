...
<header class="site-header">
    <div class="header-inner">
        ...
        <nav class="desktop-nav" aria-label="Main navigation">
            ...
            @foreach($sections as $section) @if($section['nav'])<a href="{{ in_array($section['id'],['about','business','capabilities']) ? route('corporate.'.$section['id'].'.index') : (($section['id']==='projects' ? route('projects.index') : (request()->routeIs('home') ? '' : route('home')).'#'.$section['id'])) }}">{{ $section['nav'] }}</a>@endif @endforeach
        ...
        <details class="mobile-nav"><summary>Menu <span aria-hidden="true">＋</span></summary><nav aria-label="Mobile navigation"><a href="{{ route('home') }}">Home</a>@foreach($sections as $section) @if($section['nav'])<a href="{{ in_array($section['id'],['about','business','capabilities']) ? route('corporate.'.$section['id'].'.index') : (($section['id']==='projects' ? route('projects.index') : (request()->routeIs('home') ? '' : route('home')).'#'.$section['id'])) }}">{{ $section['nav'] }}</a>@endif @endforeach @foreach($menuItems->whereIn('placement',['header','both']) as $item)<a href="{{ $item['url'] }}">{{ $item['title'] }}</a>@endforeach</nav></details>
    </div>
</header>
...
<footer class="site-footer">
    ...
    <nav class="footer-nav" aria-label="Footer navigation">@foreach($sections as $section) @if($section['nav'])<a href="{{ in_array($section['id'],['about','business','capabilities']) ? route('corporate.'.$section['id'].'.index') : (($section['id']==='projects' ? route('projects.index') : (request()->routeIs('home') ? '' : route('home')).'#'.$section['id'])) }}">{{ $section['nav'] }}</a>@endif @endforeach @foreach($menuItems->whereIn('placement',['footer','both']) as $item)<a href="{{ $item['url'] }}">{{ $item['title'] }}</a>@endforeach</nav>
